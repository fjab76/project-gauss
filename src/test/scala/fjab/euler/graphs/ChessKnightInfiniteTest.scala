...
class ChessKnightInfiniteTest extends FunSuite {
  ...
  test("should be List((1, 2), (3,3), (5,4)) for path (1,2)->(5,4)") {
      assert(new ChessKnightInfinite((5,4)).findShortestPathFrom((1,2)) == List((1, 2), (3,3), (5,4)))
    }

  test("should be List() for path (0,0)->(8,1)") {
      assert(new ChessKnightInfinite((8,1)).findShortestPathFrom((0,0)) == List((0,0), (2,1), (4,2), (5,4), (6,2), (8,1)))
    }
